...
function fetchAllUsers()
{
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		FROM ".$db_table_prefix."users");
	$stmt->execute();
	$stmt->bind_result($id,$idktp, $user, $lengkap, $password,$sex,$blood ,$birthplace,$birthday,$rt,$rw,$alamat,$nikah,$agama,$pekerjaan,$penghasilan,$miskin, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn);
	
	while ($stmt->fetch()){
		$row[] = array('id' => $id,  'id_ktp' => $idktp,'user_name' => $user, 'nama_lengkap' => $lengkap, 'password' => $password, 'jenis_kelamin' => $sex, 'golongan_darah' => $blood, 'tempat_lahir' => $birthplace, 'tanggal_lahir' => $birthday,'rt' => $rt,'rw' => $rw,'alamat' => $alamat,'status_nikah' => $nikah,'agama' => $agama,'pekerjaan' => $pekerjaan,'penghasilan' => $penghasilan,'status_miskin' => $miskin,'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn);
	}
	$stmt->close();
	return ($row);
}
...
function fetchUserDetails($username=NULL,$namalengkap=NULL,$token=NULL, $id=NULL)
{
	if($username!=NULL) {
		$column = "user_name";
		$data = $username;
...
function namalengkapExists($namalengkap)
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("SELECT active
		FROM ".$db_table_prefix."users
